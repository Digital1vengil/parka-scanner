...
//
// NOTA: esta es una copia espejo. La fuente de verdad es el proyecto de Apps Script.
// El proyecto real tiene ademas el hook de bicis-stock en doPost y el archivo
// BicicletasStock.gs, que no estan en esta copia.
...
function doPost(e) {
  ...
  try {
    ...
    } else if (action === 'cerrarDia') {
      return jsonOut(generarCierreDelDia(body.fecha || null));
...
function procesarYGuardarExcel(body, tipo) {
  ...
  // Log del dia para el cierre unico
  registrarEnLog(datos, tipo, fecha);
  ...
  // 5. Guardar .xlsx en la subcarpeta de parciales dentro de la carpeta tipo/fecha
  var finFolder = getParcialesFolder(tipo);   // parciales; el archivo unico va a 'Cierre del dia'
...
function listDriveFiles(tipo) {
  ...
  // Solo la carpeta del tipo elegido; omite carpetas vacias y unifica duplicados por fecha.
  var byName = {};
  var order = [];
